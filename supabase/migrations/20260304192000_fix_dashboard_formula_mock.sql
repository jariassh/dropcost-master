-- Migration: Fix dashboard KPIs averages based on individual costeo metrics directly from costeos table
-- Date: 2026-03-04
-- Author: AI Assistant
...
BEGIN
    ...
    WITH costeo_metrics AS (
        SELECT 
            c.id,
            c.precio_final,
            COALESCE(c.meta_spend, 0) as real_spend,
            COALESCE(c.meta_cvr, 0) as cvr_real,
            (
               SELECT COUNT(*)
