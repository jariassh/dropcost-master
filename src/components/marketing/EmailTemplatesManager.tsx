...
const categorizedVariables = {
    ...
    'Financiero': [
        { name: 'producto_nombre', label: 'Nombre del Producto (Ãltimo)' },
    ...
    'Marca & Colores': [
        ...
        { name: 'color_success', label: 'Color Ãxito' },
...
export function EmailTemplatesManager() {
    ...
    // Triggers logic is now handled by useEmailTriggers hook (see below)
    ...
    async function handleSendTestEmail() {
        ...
        try {
            ...
            if (resultado.success) {
                toast.success('Â¡Enviado!', `Correo de prueba enviado a ${selectedTestUser.email}`);
            } else {
    ...
    const { data: triggers = [] } = useEmailTriggers();
    ...
    async function handleSave() {
        ...
        try {
            ...
            toast.success('Â¡Guardado!', 'La plantilla se ha actualizado correctamente.');
        } catch (error) {
    ...
    async function handleCreateTemplate() {
        ...
        try {
            ...
            toast.success('Â¡Creado!', 'La plantilla se ha creado correctamente.');
    ...
    async function handleCreateFolder() {
        ...
        try {
            ...
            setNewItem({ name: '', slug: '', description: '', subject: '', trigger_event: '', mjml_content: '', sender_prefix: 'support' });
            toast.success('Â¡Creado!', 'La carpeta se ha creado correctamente.');
        } catch (error) {
    ...
    async function handleClone(item: EmailItem) {
        try {
            ...
            toast.success('Â¡Clonado!', 'La plantilla se ha clonado correctamente.');
        } catch (error) {
    ...
    const renderPreview = (content: string) => {
        ...
        if (selectedUserPlan) {
            ...
            fallbacks['plan_detalles'] = Array.isArray(selectedUserPlan.features)
                ? selectedUserPlan.features.map((f: string) => `â¢ ${f}`).join('<br>')
                : '{{plan_detalles}}';
        }
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '32px', animation: 'fadeIn 0.4s ease-out' }}>
            <style>{styles}</style>
            {selectedTemplate && (
                <div className="dc-admin-header-row" style={{ display: 'flex', flexDirection: 'column', gap: '24px', marginBottom: '16px' }}>
                    <div style={{ display: 'flex', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '20px' }}>
                        <div>
                            <h2 style={{ fontSize: '18px', fontWeight: 600, color: 'var(--text-primary)', margin: 0 }}>
                                Editando: {selectedTemplate.name || selectedTemplate.slug}
                            </h2>
                        </div>
            ...
            {!selectedTemplate ? (
                <React.Fragment>
                    ...
                    <div style={{
                        ...
                        <div style={{ display: 'flex', gap: '12px', flex: 1, maxWidth: isMobile ? '100%' : '700px', flexDirection: isMobile ? 'column' : 'row' }}>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '12px', flexWrap: 'wrap' }} className="dc-admin-actions-container">
                            <div style={{ display: 'flex', gap: '4px', backgroundColor: 'var(--bg-secondary)', padding: '4px', borderRadius: '10px' }}>
                                <button
                    ...
                    {navigationPath.length > 0 && (
                        <div className="flex items-center gap-3 px-2 py-3 text-[11px] font-bold uppercase tracking-wider text-[var(--text-tertiary)] animate-in fade-in slide-in-from-left-2 duration-300">
                            <span className="hover:text-[var(--color-primary)] cursor-pointer transition-colors" onClick={() => setNavigationPath([])}>DIRECTORIO RAÃZ</span>
                ...
                <div className="flex flex-col gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                    ...
                    {/* SECCIÃN VISTA PREVIA: Siempre visible y Responsiva */}
                    <Card
