...
export const auditService = {
   ...
   */
  async logUserActivity(accion: string, detalles: any = {}, entidad: any = 'SYSTEM', entidad_id: any = null) {
   ...
   */
  async getLogs(filters: AuditFilters = {}, page = 1, limit = 50): Promise<{ data: AuditLog[], count: number }> {
    try {
      let query = supabase
        .from('audit_logs')
        .select(`
   ...
   */
  async fetchUserActivityLogs(limit = 20): Promise<AuditLog[]> {
      try {
          const { data: { user } } = await supabase.auth.getUser();
          if (!user) return [];
